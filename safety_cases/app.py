            ...
            [
                ...
                dbc.Col(
                    [
                        ...
                        html.Hr(),
                        dbc.Input(id="delete-node-id", placeholder="Node ID to Delete"),
                        dbc.Button("Delete Node", id="delete-node-btn", color="danger"),
                    ],
                    md=4,
                ),
            ]
...
@app.callback(
    [Output("node-id", "value"), Output("node-label", "value"), Output("node-text", "value")],  # Added node-text output
...
@app.callback(
    ...
    [State("node-id", "value"), State("node-label", "value"), State("node-text", "value"), State("cytoscape", "elements"),  # Added node-text state
     ...
     State("target-node-id", "value"), State("delete-node-id", "value")]
)
def update_elements(n_clicks_update, n_clicks_add_node, n_clicks_add_edge, n_clicks_delete_node,
                    ...
                    new_node_text, new_node_shape, new_node_color, source_node_id, target_node_id, delete_node_id):
    ...
    elif button_id == "delete-node-btn" and delete_node_id:
        elements = [element for element in elements if element["data"]["id"] != delete_node_id and element["data"].get("source") != delete_node_id and element["data"].get("target") != delete_node_id]
